...
mod proof {
    use super::*;
    #[kani::proof]
    fn legal_moves_should_place_kani() {
    ...
    #[kani::proof]
    fn illegal_moves_should_not_place_kani() {
    ...
    #[kani::proof]
    fn legal_moves_should_be_legal_kani() {
        // Check so that all legal moves returned can be individually verified as legal

        let game = Game::try_from(kani::any::<ShadowGame>()).unwrap();

        assert!(game.moves().hot_bits().all(|pos| game.is_legal_move(pos)));
    }

    #[kani::proof]
    fn illegal_moves_should_be_illegal_kani() {
        // Check so that all moves not contained in the set of legal moves
        // actually is illegal
        let game = Game::try_from(kani::any::<ShadowGame>()).unwrap();
